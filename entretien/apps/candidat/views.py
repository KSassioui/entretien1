...
from django.core.mail import send_mail
from django.conf import settings
from apps.entretien.models import Entretien, ReponseEntretien  # ✅ Ajouté ReponseEntretien
from django.shortcuts import get_object_or_404
...
def candidats_by_offer(request, offer_id):
    ...
    Inclut l'UUID de l'entretien pour permettre l'accès aux vidéos.
    """
    applications = Application.objects.filter(job_offer_id=offer_id).select_related(
        'candidat__user'
    )

    data = []
    for app in applications:
        # 🔍 Récupérer l'entretien lié à ce candidat et cette offre
        entretien = Entretien.objects.filter(
            candidat=app.candidat,
            offre=app.job_offer
        ).first()

        # ✅ Vérifier s'il y a des réponses vidéo
        has_video = False
        if entretien:
            has_video = ReponseEntretien.objects.filter(entretien=entretien).exists()

        data.append({
            "id": app.id,
            "email": app.candidat.user.email,
            "interviewStatus": "Enregistré" if has_video else "Non Enregistré",
            "requestStatus": "Lien envoyé" if app.has_video else "Pas de demande",
            "entretien_uuid": str(entretien.lien_unique) if entretien else None,  # ✅ UUID ajouté
        })
...
#             "interviewStatus": "Enregistré" if has_video else "Non Enregistré",
#             "requestStatus": "Lien envoyé" if app.has_video else "Pas de demande",
#             "entretien_uuid": str(entretien.lien_unique) if entretien else None,
#         })
...
#     # ✅ Mettre à jour : le lien a été envoyé
#     application.has_video = True  # 🔑 C’est ici que le statut change
#     application.save()  # ✅ Obligatoire
